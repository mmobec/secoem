...
\subsubsection{Hydrogen Demand and Hydrogen Sold}
\label{SEC:HydrogenDemand}

We assume that the hydrogen has to be delivered at a pressure of \(Pr^{\rm DEM}_{\rm H2}\) for both refueling station demand and sales. The routing logic that determines which of the three pathways (\textit{EL}, \textit{CP}, \textit{HS}) is admissible to deliver hydrogen at each time step has been detailed in \ref{SEC:HydrogenChain}.
...
\begin{equation} \label{EQ:IBDef}
  ...
  - \\
  \Bigl(e^{DA+}_{t,\omega} + \sum_{i \in \mathcal{I}_t} e^{IM}_{i,t,\omega}
      + fd_{t,\omega} + c_{t,\omega}
      + e^{EL}_{t,\omega} + e^{CP}_{t,\omega} + e^{FC,sb}_{t,\omega}\Bigr) \quad \forall t \in \mathcal{T}, \ \forall \omega \in \Omega.
\end{equation}
